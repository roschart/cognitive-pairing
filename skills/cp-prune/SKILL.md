...
description: >
  ...
  checkpoints. Use when memory exceeds 1500 words despite compaction,
  a major phase has completed, the project has pivoted, or old
  checkpoints clutter the directory. This is maintenance, not routine.
metadata:
  author: roschart
  version: "1.0"
---
...
- `.cp/memory/active.md` exceeds 1500 words despite recent compaction
...
1. The current `.cp/memory/active.md`
2. The `plan-<slug>.md` (to verify what is still active)
...
- Produces a pruned `.cp/memory/active.md` (human reviews before
  replacing)
- Optional: identifies checkpoints in `.cp/checkpoints/` that can be
  archived or deleted
...
Review .cp/memory/active.md against the current plan-<slug>.md and
latest checkpoint. Identify and remove content that no longer serves
active work.
...
Specifically prune:
  - Decisions that are stable and no longer in flux (move to
    checkpoint; they do not need to stay in active memory)
...
Output:
  1. A pruned .cp/memory/active.md
...
Old checkpoints do not need to be deleted. If they are no longer
useful as recovery anchors but you want to preserve them:
