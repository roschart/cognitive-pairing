...
From these, build an internal analysis:

- Full active.md content: verbatim.
- Latest checkpoint — current state: Current State and
  Pending Work sections only, verbatim.
- Active plans — task status: for each plan found, plan
  name + full task list with current check states.
- Canon facts: full list — items in active.md that
  duplicate canon should be removed.

### How the main agent uses this

1. **Read the files directly** and build the analysis above.
2. **Apply the relevance test** to each item: "if missing,
   would it affect current or near-future work?"
3. **Produce pruned `active.md` draft + removal list**
...
1. **Read `.cp/` files directly** (see File reading above)
   and build the internal analysis.
